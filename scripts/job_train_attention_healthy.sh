...
#PBS -N train_attention_healthy
#PBS -o $PBS_O_WORKDIR/logs/train_attention_healthy.out
#PBS -e $PBS_O_WORKDIR/logs/train_attention_healthy.err
...
echo "Training healthy (attention U-Net) | Start: $(date)"
python train.py --class_idx 0 \
    --epochs 1000 \
    --save_every 50 \

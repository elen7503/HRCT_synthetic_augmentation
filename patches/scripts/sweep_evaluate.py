...
SWEEP_DIR = os.path.join(PROJECT_ROOT, f"diffusion_model/outputs/checkpoint_sweep/{CLASS_NAME}")
...
def main():
    ...
    for entry in manifest:
        ...
        print(f"{entry['checkpoint']:15s}  epoch={entry['epoch']:5}  train_loss={entry['train_loss']:.4f}  "
    ...
    print(f"\nSaved to {OUT_CSV}, sorted by recall (highest first):")
    print(df.to_string(index=False))
    print(f"\nbest.pt was selected by lowest train_loss. Compare its rank here against")
    print(f"checkpoints with higher recall/precision but higher train_loss --")
    print(f"if an earlier epoch scores better, that's a free improvement (no retraining needed).")
